<div id="passwordModal" x-data="userPasswordModal(@js($hasPasswordErrors))" x-show="isOpen" x-on:open-password-modal.window="openModal($event)" :class="{ 'flex': isOpen, 'hidden': !isOpen }" class="hidden fixed inset-0 z-[100] items-center justify-center p-4" x-cloak>
        ...
        <div class="px-6 pb-6">
            <form action="{{ route('user-profile.updatePassword', $user->user_id) }}" method="POST" id="passwordForm" class="space-y-5" @submit="submitPassword()">
                ...
                <div class="flex gap-3 pt-4">
                    <button type="button" @click="closeModal()" class="flex-1 flex justify-center items-center gap-3 px-4 py-2 font-bold text-lg text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-slate-700 rounded-xl transition-all group">
