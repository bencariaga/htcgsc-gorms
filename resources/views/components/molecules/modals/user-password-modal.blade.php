...
<div id="passwordModal" x-data="{
    showErrors: {{ $hasPasswordErrors ? 'true' : 'false' }},
    submit() {
        const modal = document.getElementById('passwordModal');

        if (modal) modal.classList.replace('flex', 'hidden');

        const loader = document.getElementById('loadingPassword');
...
}" x-init="if (showErrors) { setTimeout(() => showErrors = false, 5000) }" class="{{ $hasPasswordErrors ? 'flex' : 'hidden' }} fixed inset-0 z-[100] items-center justify-center p-4">
    ...
    <div class="max-w-md w-full relative z-10 bg-white dark:bg-slate-800 shadow-xl border border-gray-300 dark:border-slate-700 rounded-2xl overflow-hidden">
        ...
        <div class="px-6 pb-6">
            <form action="{{ route('user-profile.updatePassword', $user->user_id) }}" method="POST" id="passwordForm" class="space-y-5" @submit="submit()">
                @csrf
