<div class="fixed bottom-6 right-6 z-[150] flex flex-col gap-4 min-w-[25rem]">
    <template x-for="note in notifications" :key="note.id">
        <div x-init="setTimeout(() => { notifications = notifications.filter(n => n.id !== note.id) }, 5000)"
            ...
            class="relative overflow-hidden bg-white dark:bg-slate-800 p-4 rounded-lg shadow-2xl flex items-center justify-between"
            :class="{
            ...
            <div class="flex items-center space-x-4">
                ...
                <div class="flex flex-col">
                    <span class="text-slate-800 dark:text-white font-bold font-base capitalize" x-text="note.type"></span>
                    <span class="text-slate-600 dark:text-slate-300 font-medium text-[14px] pr-[22px]" x-html="note.message"></span>
                </div>
            </div>

            <button @click="notifications = notifications.filter(n => n.id !== note.id)" class="ml-4 text-slate-400 hover:text-slate-600 dark:hover:text-white transition-colors">
