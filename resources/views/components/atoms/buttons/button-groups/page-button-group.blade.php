@foreach ($styleEnum->getMenuItems() as $item)
    ...
    @if ($isHeader)
        <a href="{{ route($route) }}" class="text-lg {{ $item['width'] }} flex justify-between items-center px-4 py-2 font-semibold rounded-xl transition-all group {{ request()->routeIs($route) ? 'text-blue-700 dark:text-blue-400 bg-blue-100 dark:bg-slate-700' : 'hover:text-blue-700 dark:hover:text-blue-400 hover:bg-blue-100 dark:hover:bg-slate-700' }}">
            <i class="fas {{ $item['icon'] }} mr-2.5 {{ request()->routeIs($route) ? 'text-blue-500' : 'text-slate-400' }} group-hover:scale-110 group-hover:text-blue-500"></i>
            <span>{{ $item['label'] }}</span>
        </a>
    @else
        <li class="relative flex items-center group">
            <a href="{{ route($route) }}" class="flex items-center w-full px-3 py-2 rounded-lg {{ request()->routeIs($route) ? 'bg-white/10 text-yellow-300' : 'text-white' }} hover:bg-white/10 hover:text-emerald-300 transition-colors">
                <div class="w-8 h-8 flex justify-center items-center shrink-0 ml-0 mr-5">
                    <i class="fas {{ $item['icon'] }} text-lg {{ request()->routeIs($route) ? 'text-yellow-300' : 'text-white' }} group-hover:scale-110 group-hover:text-emerald-400 transition-transform"></i>
                </div>
