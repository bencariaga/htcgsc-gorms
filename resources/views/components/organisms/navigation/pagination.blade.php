    ...
    @if($paginator->hasPages())
        <nav role="navigation" aria-label="Pagination Navigation" class="flex items-center gap-2">
            @if(!$paginator->onFirstPage())
                <button type="button" wire:click="previousPage" wire:loading.attr="disabled" class="flex items-center justify-center h-[2.5rem] w-[2.5rem] text-emerald-600 bg-white dark:bg-slate-900 border-2 border-gray-400 dark:border-slate-600 rounded-lg hover:border-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-all shadow-sm">
                ...
                @foreach ($paginator->render()->getData()['elements'] as $element)
                    @if(Validator::make(['element' => $element], ['element' => 'string'])->passes())
                        <span aria-disabled="true">
                            <span class="flex items-center justify-center h-[2.5rem] w-[2rem] text-gray-700 dark:text-white font-semibold text-xl">{{ $element }}</span>
                        </span>
                    @endif
            ...
            @if($paginator->hasMorePages())
                <button type="button" wire:click="nextPage" wire:loading.attr="disabled" class="flex items-center justify-center h-[2.5rem] w-[2.5rem] text-emerald-600 bg-white dark:bg-slate-900 border-2 border-gray-400 dark:border-slate-600 rounded-lg hover:border-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-all shadow-sm">
