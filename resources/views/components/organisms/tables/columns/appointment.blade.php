...
<td class="{{ $cellStyling }}">
    <span class="text-base font-bold text-slate-700 dark:text-slate-200 tracking-[0.5px] tabular-nums">
        <div class="whitespace-nowrap">
            {{ $item->formatted_appointment_id }}
        </div>
    </span>
</td>

<td class="{{ $cellStyling }} {{ $isReschedulable ? 'group relative cursor-pointer' : '' }}" @if($isReschedulable) x-data x-on:click="$dispatch('open-reschedule-modal', { id: 'rescheduleAppointmentModal', appointmentId: {{ $item->appointment_id }}, formattedId: '{{ $item->formatted_appointment_id }}', studentName: '{{ $person?->full_name }}', currentDate: '{{ $modalBookedTime }}' })" @endif>
...
<td class="{{ $cellStyling }}">
    <x-atoms.utility.status-dot :status="$item->appointment_status" />
</td>
