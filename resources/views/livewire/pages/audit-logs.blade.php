...
<div class="flex h-full overflow-hidden">
    ...
    <main class="flex-1 flex flex-col min-w-0 overflow-y-auto [scrollbar-width:thin] [scrollbar-color:rgba(107,114,128,0.8)_transparent] [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-gray-500/80">
        <x-pages.list-type :items="$logs" type="audit-log" :perPage="$perPage" sortField="datetime" sortDirection="desc" idColumn="datetime" alphaColumn="message" :filter="$filter" :selectedFileName="$selectedFileName" :columns="['Type', 'Date and Time', 'Message']" groupClass="flex-col 2xl:flex-row justify-between gap-3" />
    </main>
</div>
